...
```bash
cd backend
go run ./scripts/gen_jmeter_data -count 1000 # 与启用的线程组一致
```

输出默认：`tests/jmeter/out/jmeter_users.csv`
...
- `out/results-<时间戳>.jtl`
- `out/jmeter-<时间戳>.log`
- `out/report-<时间戳>/index.html`

### 3. 高并发

以 3000 线程并发为例：

1. 在 JMeter GUI 或编辑 `enrollment-load.jmx`，启用相应配置（默认是1000线程组）
2. `gen_jmeter_data -count 3000` 生成 3000 行 CSV
3. 执行 `run-jmeter-report.sh`
